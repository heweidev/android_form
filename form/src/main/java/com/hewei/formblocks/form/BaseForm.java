...
public class BaseForm {
    ...
    public void add(LinearLayout container) {
        ...
        for (Field field : fields) {
            ...
            if (block != null) {
                ...
                Object object = onSetup(id, itemType, clsFactory, size,
                        new ListArgProvider(Arrays.asList(block.args())), container);
                if (object != null) {
                    try {
    ...
    public void add(BlockProvider provider, LinearLayout container) {
        if (provider == null) {
            return;
        }
    ...
    public void add(BatchProvider provider, LinearLayout container) {
        if (provider == null) {
            return;
        }
    ...
    Object onSetup(String id, Class<?> itemType, Class<? extends BlockFactory<?>> clsFactory,
                   int size, ArgsProvider args, LinearLayout container) {
        BaseBlock<?> block;
        if (size == 1) {
            try {
    ...
    /**
     * 从表单获取数据，这里采用的是遍历cls的成员，然后从表单取数据的方法。
     * 所以，如果成员中有较多非表单数据，则效率较低。
     * 反之，采用从表单获取数据然后设置到成员，会出现数据不兼容的问题。
    ...
    public <T> T getData(Class<T> cls) {
        ...
        for (Method m : methods) {
            ...
            String name = Utils.tripPrefix("set", methodName);
